...
public class GroovyTemplateCompiler extends TemplateCompiler {
    ...
    public BaseTemplate compile(BaseTemplate template) {
        try {
            ...
            extensionsClassnames.addAll(TemplateEngine.engine.addTemplateExtensions());
            List<Class> extensionsClasses = TemplateEngine.utils.getAssignableClasses(JavaExtensions.class);
    ...
    String source() {
        ...
        source = TemplateEngine.engine.overrideTemplateSource(template, source);

        // Static access
        List<String> names = new ArrayList<String>();
        Map<String, String> originalNames = new HashMap<String, String>();
        for (Class clazz : TemplateEngine.utils.getAllClasses()) {
    ...
    void head() {
        ...
        String uniqueNumberForTemplateFile = GenericTemplateLoader.getUniqueNumberForTemplateFile(template.name);
    ...
    void endTag() {
        ...
        if (tag.name.equals("doBody")) {
            ...
            try {
                ...
                try {
                    fastClasses = TemplateEngine.utils.getAssignableClasses(FastTags.class);
                } catch (Exception xe) {
                    //
                }
                fastClasses.add(0, FastTags.class);
                fastClasses.addAll(1, TemplateEngine.engine.getFastTags());
                Method m = null;
                ...
                for (Class<?> c : fastClasses) {
                    ...
                    FastTags.Namespace a = c.getAnnotation(FastTags.Namespace.class);
                    // play internal FastTags
                    if (c.isAnnotationPresent(FastTags.Namespace.class) && a != null && !(a.value().equals(tSpace) || (tSpace.length() == 0 && a.value().equals("_play")))) {
                        continue;
                    }
                    try {
